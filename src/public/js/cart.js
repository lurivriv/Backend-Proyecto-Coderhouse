...
if (tableCartContainer) {
    ...
    updateProductQuantityInCartBtn.forEach(async (btn) => {
        btn.addEventListener("click", async (e) => {
            try {
                ...
                const productPrice = parseInt(e.currentTarget.getAttribute("data-product-price"))
                ...
                const newTotalPrice = Array.from(updatedSubtotalPrices).reduce((acc, elem) => acc + parseInt(elem.innerText.replace("$", "")), 0)
    ...
    const deleteAllProductsInCartBtn = document.querySelector(".delete-all-products-in-cart")
